...
public class C02_Previos {
    public static void main(String[] args) {
        List<Integer>l1=new ArrayList<>(Arrays.asList(24,38,65,9,11,7,27,23));
        //l1 elemanlarını tersten prınt ediniz
        System.out.println("l1 lıst ilk hali " +l1);
        /*
        tekrarda son elemandn baslayan işlemler için pointer en sona goturmeli
        bunun için poınter e sona goturen hasNext() meth. çalışmalı.
         */
        ListIterator<Integer>it1=l1.listIterator();
        while (it1.hasNext()){//poınter sureklı bastan sona
            it1.next();

        }
        while (it1.hasPrevious()){//poınter sureklı sondan basa
            System.out.print(it1.previous()+" ");//23 27 7 11 9 65 38 24
        }
        //System.out.println("l1 lıst son hali " +l1);
        System.out.println("   ***   ");


        List<String>l2=new ArrayList<>(Arrays.asList("cagla","ugur","muharrem","cebrail"));
        //l2 elemanlarını sonuncusuna :-) ekleyıp prınt ediniz
        System.out.println("lıstıterator oncesi l2:" +l2);
        ListIterator<String>it2=l2.listIterator();//l2 elemanarını tekrarlayan lıstıterator tanımlandı
        while (it2.hasNext()){
           String s= it2.next();
